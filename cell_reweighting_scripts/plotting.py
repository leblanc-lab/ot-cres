...
def plot_same_rw_all(obs, df, weights_orig, numbins, xmin, xmax, obs_str = "", sel=None, ymin=1e-4, ymax=1e0, logy=True, title="TTbar", channel="zjets_100k",raxlim=[0.85, 1.15]):
    if sel is None:
    ...
    plt.savefig(f"../plots/{channel}/{title}_{obs_str}.png")
    plt.show()

def plot_diff_rw(obs, dfs, strings, fracs, weights_orig, xmin, xmax, nbins, ymin=1e-5, ymax=1e0, obs_str = "", sel=None, title="", raxlim=[0.85, 1.15], channel = "zjets_100k"):
    if sel is None:
        sel  = np.ones_like(weights_orig, dtype=bool)
    fig, (ax, rax) = plt.subplots(nrows=2,
                        ncols=1,
