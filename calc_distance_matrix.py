...
calc_emds = wasserstein.EMDYPhi(R=1, 
                                        beta=1,
                                        norm=False,
                                        #num_threads=-1,
                                        #print_every=1000,
...
hardprocess_dist2neg = np.zeros((len(neg_events), len(hardprocess_EMD_pt[:,0])))

def hardprocess_calc_emds_multiproc(i, j, hardprocess_EMD_pt, hardprocess_EMD_etaphi):
    if i % 100 == 0 and j == 9990:
        print(f'{np.where(np.array(neg_events) == i)[0].item()}th row computed')
    try:
        return calc_emds(hardprocess_EMD_pt[i], hardprocess_EMD_etaphi[i], 
                     hardprocess_EMD_pt[j], hardprocess_EMD_etaphi[j])
...
with mp.Pool(processes=mp.cpu_count()) as pool:
    results = pool.starmap(
        hardprocess_calc_emds_multiproc, 
        [(i, j, hardprocess_EMD_pt, hardprocess_EMD_etaphi) for i in neg_events for j in range(len(hardprocess_EMD_pt[:,0]))]
    )

hardprocess_dist2neg = np.array(results).reshape(len(neg_events), len(hardprocess_EMD_pt[:,0]))

np.savetxt('hardprocess_dist2neg.csv', hardprocess_dist2neg, delimiter=',')
...
showered_dist2neg = np.zeros((len(neg_events), len(showered_EMD_pt[:,0])))

def shower_calc_emds_multiproc(i, j, showered_EMD_pt, showered_EMD_etaphi):
    if i % 100 == 0 and j == 9990:
        print(f'{np.where(np.array(neg_events) == i)[0].item()}th row computed')
    try:
        return calc_emds(showered_EMD_pt[i], showered_EMD_etaphi[i], 
                     showered_EMD_pt[j], showered_EMD_etaphi[j])
...
with mp.Pool(processes=mp.cpu_count()) as pool:
    results = pool.starmap(
        shower_calc_emds_multiproc, 
        [(i, j, showered_EMD_pt, showered_EMD_etaphi) for i in neg_events for j in range(len(showered_EMD_pt[:,0]))]
    )

showered_dist2neg = np.array(results).reshape(len(neg_events), len(showered_EMD_pt[:,0]))

np.savetxt('showered_dist2neg.csv', showered_dist2neg, delimiter=',')
...
hadronization_dist2neg = np.zeros((len(neg_events), len(hadronization_EMD_pt[:,0])))

def hadron_calc_emds_multiproc(i, j, hadronization_EMD_pt, hadronization_EMD_etaphi):
    if i % 100 == 0 and j == 9990:
        print(f'{np.where(np.array(neg_events) == i)[0].item()}th row computed')
    try:
        return calc_emds(hadronization_EMD_pt[i], hadronization_EMD_etaphi[i], 
                     hadronization_EMD_pt[j], hadronization_EMD_etaphi[j])
    except RuntimeError as e:
        print(f"Error with EMD calculation between event {i} and {j}: {str(e)}")
        return 1000
...
with mp.Pool(processes=mp.cpu_count()) as pool:
    results = pool.starmap(
        ...
        [(i, j, hadronization_EMD_pt, hadronization_EMD_etaphi) for i in neg_events for j in range(len(hadronization_EMD_pt[:,0]))]
    )

hadronization_dist2neg = np.array(results).reshape(len(neg_events), len(hadronization_EMD_pt[:,0]))

np.savetxt('hadronization_dist2neg.csv', hadronization_dist2neg, delimiter=',')
